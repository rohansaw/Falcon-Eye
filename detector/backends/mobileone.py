...
# This code is copied and modified from https://github.com/apple/ml-mobileone
...
class MobileOne(nn.Module):
    ...
    def __init__(
        ...
        print("MobileOne backend initialized. Using NOT truncated model!")

    def truncate(self, last_stage_num: int = 2):
        """Method to truncate the model to a specified number of stages.

        :param last_stage_num: Number of stages to keep.
        """
        self.last_stage_num = last_stage_num
        if last_stage_num == 2:
            self.stage3 = None
            self.stage4 = None
            self.gap = None
            self.linear = None
        elif last_stage_num == 3:
            self.stage4 = None
            self.gap = None
            self.linear = None
        elif last_stage_num == 4:
            self.gap = None
            self.linear = None
        else:
            raise ValueError("Invalid number of stages to keep. Has to be between 2 and 4.")
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply forward pass."""
        x = self.stage0(x)
        # print("0", sum(p.numel() for p in self.stage0.parameters()))
        # print("stage 0 out shape: ", x.shape)
        x = self.stage1(x)
        # print("1", sum(p.numel() for p in self.stage1.parameters()))
        # print("stage 1 out shape: ", x.shape)
        x = self.stage2(x)
        ...
        if self.stage4 is not None:
            x = self.stage4(x)
        #x = self.stage3(x)
        # print("stage 3 out shape: ", x.shape)
        # x = self.stage4(x)
        # # print("4", sum(p.numel() for p in self.stage4.parameters()))
        # # # # # print("stage 4 out shape: ", x.shape)
        if not self.last_stage_num:
            x = self.gap(x)
            x = x.view(x.size(0), -1)
            x = self.linear(x)
        # x = self.gap(x)
        # # print("gap", sum(p.numel() for p in self.gap.parameters()))
        # x = x.view(x.size(0), -1)
        # x = self.linear(x)
        # print("Linear", sum(p.numel() for p in self.linear.parameters()))
        return x
...
def mobileone(
    num_classes: int = 1000, inference_mode: bool = False, variant: str = "s0", bn_momentum: float = 0.9
) -> nn.Module:
    """Get MobileOne model.
    ...
    return MobileOne(
        num_classes=num_classes, inference_mode=inference_mode, bn_momentum=bn_momentum, **variant_params
    )
