...
coco_tile_pred_path = "/home/thesis/tiny-od-on-edge/baselines/efficientdet/testdev_output_sw_sliced_val/detections_test-dev2017_test_results.json"
coco_tile_gt_path = "/data/sw_sliced/coco/annotations/instances_val.json"
coco_tile_gt = None
coco_tile_pred = None
with open(coco_tile_gt_path, "r") as f:
    coco_tile_gt = json.load(f)
    
with open(coco_tile_pred_path, "r") as f:    
    coco_tile_pred = json.load(f)
...
fn_tile_id = {}
for img in coco_tile_gt["images"]:
    # fn_tile_id[remove_idx(img["file_name"])] = img["id"] # onyl for sds_sliced_new!!!
    fn_tile_id[img["file_name"]] = img["id"]
    
tile_preds = {}
for pred in coco_tile_pred:
    iid = pred["image_id"]
    if iid not in tile_preds:
        tile_preds[iid] = []
    tile_preds[iid].append(pred)
    
def get_tile_id(tile_fn):
    if not tile_fn in fn_tile_id:
        #print(f"Image {tile_fn} not found in tile ground truth")
        return None
    return fn_tile_id[tile_fn]

def get_tile_preds(tile_id):
    if not tile_id in tile_preds:
        return []
    return tile_preds[tile_id]
...
def get_sliced_prediction(
    ...
    for group_ind in range(num_group):
        ...
        cur_object_prediction_list = []
       
        
        for offset in shift_amount_list:
            img_h = slice_image_result.original_image_height
            img_w = slice_image_result.original_image_width
            tile_end_x = min(offset[0] + slice_width, img_w)
            tile_end_y = min(offset[1] + slice_height, img_h)
            tile_file = f"{image_name}_{offset[0]}_{offset[1]}_{tile_end_x}_{tile_end_y}.png"
            tile_coco_id = get_tile_id(tile_file)
            if tile_coco_id is None:
                continue
            preds = get_tile_preds(tile_coco_id)
            for pred in preds:
                ...
                cur_object_prediction_list.append(object_prediction)
            
        
        
                
        # convert sliced predictions to full predictions
        for object_prediction in cur_object_prediction_list:
