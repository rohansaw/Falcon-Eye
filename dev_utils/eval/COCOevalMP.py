...
# Adapted and extended from OPENMMLab
import copy
import itertools
import time
...
class COCOevalMP(COCOeval):
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            ...
            for area_rng in ('all', 'tiny', 'small', 'medium', 'large'):
                stats.append(
                    _summarize(
                        1, iouThr=.5, areaRng=area_rng, maxDets=self.params.maxDets[-1]))
            for max_det in self.params.maxDets:
                stats.append(_summarize(0, iouThr=.5, maxDets=max_det))
            for area_rng in ('all', 'tiny', 'small', 'medium', 'large'):
                stats.append(
                    _summarize(
                        0, iouThr=.5, areaRng=area_rng, maxDets=self.params.maxDets[-1]))
